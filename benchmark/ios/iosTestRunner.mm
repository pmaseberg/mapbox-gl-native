...
-(instancetype)init
{
    ...
    if (self) {
        ...
        for (uint32_t i = 0; i < bundleContents.count; i++) {
            ...
            if ([dirName isEqualToString:@"benchmark-data"]) {
        ...
        std::string basePath = std::string([documentsDir UTF8String]) + std::string("/benchmark-data");
        self.testStatus = self.runner->startTest(basePath) ? YES : NO;
        self.resultPath = [documentsDir stringByAppendingPathComponent:@"/benchmark-data/benchmark/results.json"];
